...
sehirler_verisi = {
    "Adana": ["Otobus 154 - 2 dk", "Otobus 172 - Yakin", "Itimat 1 - 15 dk"],
    "Istanbul": ["Metrobus - 1 dk", "Vapur - Kalkiyor", "Otobus 500T - Durakta"],
...
async def read_root(sehir: str = "Adana"):
    ...
    <head>
        ...
        <title>Belediyem - Akilli Terminal</title>
        ...
        <style>
            * {{ box-sizing: border-box; }}
            ...
            #map {{ flex: 3; background: rgba(255,255,255,0.08); border-radius: 20px; border: 2px solid rgba(255,255,255,0.15); }}
            .bus-list {{ width: 260px; min-width: 260px; background: rgba(255, 255, 255, 0.1); border-radius: 20px; padding: 15px; border: 1px solid rgba(255,255,255,0.2); overflow-y: auto; }}
            ul {{ list-style: none; padding: 0; }}
            li {{ background: white; color: #1e3a8a; margin-bottom: 10px; padding: 12px; border-radius: 12px; font-weight: bold; font-size: 0.85rem; }}
            select, button {{ width: 100%; padding: 10px; border-radius: 10px; border: none; background: white; color: #1e3a8a; font-weight: bold; cursor: pointer; }}
    ...
    <body>
        <div class="tv-frame">
            <div class="tv-screen">
                <div class="reklam-bandi">DUYURU: Akilli Terminal Yayinda! | Hava: {sehir} Acik | Yarın Tum Hatlar Ucretsiz!</div>
            </div>
        </div>
        <div class="wrapper">
            <div class="sidebar">
                ...
                <div id="user-icon" class="icon-display">UFO</div>
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap: 5px;">
                    <button onclick="document.getElementById('user-icon').textContent='O'">O</button>
                    <button onclick="document.getElementById('user-icon').textContent='X'">X</button>
                </div>
                <div style="text-align: center; font-weight: bold; color: #facc15; margin-top: auto; font-size: 1.1rem;">
                    Saat: <span id="clock">{simdi}</span>
                </div>
            </div>
            <div class="main-content">
                <div class="content-row">
                    <div id="map"></div>
                    <div class="bus-list">
                        <h3 style="color: #facc15; border-bottom: 2px solid #facc15; padding-bottom: 5px; margin-top:0;">Canli Durak</h3>
