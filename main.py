...
# SÜRÜM       : v16.0.0-FINAL-SAFE-MODE (TEK PARÇA)
...
# 81 İL KOORDİNAT VERİTABANI (MİLLİ ALTYAPI)
CITIES = {
...
ACTIVE_CITIES = ["Adana", "Ankara", "İstanbul", "İzmir", "Bursa"]

# --- API UCU ---
@app.get("/api/v1/fleet")
async def get_fleet(city: str = "Adana"):
    if city not in CITIES: city = "Adana"
    status = "ACTIVE" if city in ACTIVE_CITIES else "AWAITING"
    lat, lng = CITIES[city]
    
    if status == "AWAITING":
        return {"sehir": city, "durum": status, "merkez": {"lat": lat, "lng": lng}, "arac_sayisi": 0, "ortalama_hiz": 0, "gecikme_ms": random.randint(10,35), "filo": []}
    ...
    for i in range(5):
        ...
        tot += spd
        fleet.append({
            "id": f"Hat {random.randint(10, 500)}", "hedef": f"Durak {random.randint(100, 999)}",
            "kalan": "DURAKTA" if eta == 1 else f"{eta} Dk",
            "konum": {"lat": lat + random.uniform(-0.03, 0.03), "lng": lng + random.uniform(-0.03, 0.03)},
            "hiz": spd
        })
    
    fleet.sort(key=lambda x: 0 if x["kalan"] == "DURAKTA" else int(x["kalan"].split()[0]))
    return {"sehir": city, "durum": status, "merkez": {"lat": lat, "lng": lng}, "arac_sayisi": len(fleet), "ortalama_hiz": tot//len(fleet) if fleet else 0, "gecikme_ms": random.randint(10,35), "filo": fleet}
...
async def home():
    ...
    <body>
        <div class="app">
            <div class="dash">
                <div class="title">Ulaşım Matrisine <span style="color:#2563eb;">Hoş Geldin</span>,<br><div style="font-size:15px; color:#64748b; font-weight:600; margin-top:8px;">Türkiye'nin akıllı ulaşım ağına bağlandın. Rotamız neresi?</div></div>
                <div class="grid">
...
async def belediye():
    # ŞEHİRLER BURADA KUSURSUZ ŞEKİLDE ZIRHLANIYOR (BOŞ ÇIKMASI İMKANSIZ)
    opts = "".join([f"<option value='{c}'>{c}</option>" for c in sorted(CITIES.keys())])
    html = """
    <!DOCTYPE html>
    <html lang="tr">
    <head>
    ...
    <body>
        <div class="app">
            <div class="led" id="led-bar"><div class="tk-wrap"><div class="tk-text" id="led">📡 MİLLİ ALTYAPI BAŞLATILIYOR...</div></div></div>
            <div class="nav">
                <a href="/" class="bck">❮ GERİ</a>
                <select class="sel" id="citySelect">_OPTS_</select>
            </div>
            ...
            <div class="drw">
                ...
                <div class="list" id="fleet"><div style="text-align:center; padding:30px; color:#94a3b8; font-weight:bold;">🔄 Altyapı Hazırlanıyor...</div></div>
            </div>
        </div>
        <script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script>
        <script>
            let map, layer;
            let currentCity = new URLSearchParams(window.location.search).get('city') || 'Adana';
            ...
            sel.addEventListener('change', (e) => {
                ...
                window.history.pushState({}, '', '/belediye?city=' + currentCity);
                fetchData();
            });
            ...
            async function fetchData() {
                try {
                    const res = await fetch('/api/v1/fleet?city=' + currentCity);
                    const data = await res.json();
                    
                    if (!map) {
                        map = L.map('map', {zoomControl: false}).setView([data.merkez.lat, data.merkez.lng], 13);
                        L.tileLayer('https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png', {maxZoom: 19}).addTo(map);
                        layer = L.layerGroup().addTo(map);
                        setTimeout(() => map.invalidateSize(), 300);
                    } else {
                        map.setView([data.merkez.lat, data.merkez.lng], 13);
                    }
                    ...
                    if (data.durum === "AWAITING") {
                        ledBar.style.borderBottom = "2px solid #eab308"; led.style.color = "#facc15";
                        led.innerText = 🟡 MİLLİ ALTYAPI HAZIR | ${data.sehir.toUpperCase()} İÇİN DEVLET AÇIK VERİ İZNİ BEKLENİYOR... \u00A0\u00A0\u00A0\u00A0 🟡 MİLLİ ALTYAPI HAZIR...;
                        cont.innerHTML = <div style="text-align:center; padding:40px 20px; color:#475569; font-size:14px; line-height:1.6;"><b>Altyapı Entegrasyonu Tamamlandı!</b><br><br>${data.sehir} ilinin coğrafi veri tabanı eklendi. Kurum izni bekleniyor.</div>;
                    } else {
                        ledBar.style.borderBottom = "2px solid #2563eb"; led.style.color = "#38bdf8";
                        ...
                        data.filo.forEach(b => {
                            ...
                            cont.innerHTML += <div class="row item"><div class="info"><span class="id">🚌 ${b.id}</span><span class="dst">Hedef: ${b.hedef} | Hız: ${b.hiz} km/s</span></div><div class="eta" style="${bg}">${b.kalan}</div></div>;
                            L.marker([b.konum.lat, b.konum.lng], {icon: icon}).addTo(layer);
                        });
                        
                        const now = new Date().toLocaleTimeString('tr-TR');
                        led.innerText = 🟢 GÜVENLİ | ŞEHİR: ${data.sehir.toUpperCase()} | ARAÇ: ${data.arac_sayisi} | ORT. HIZ: ${data.ortalama_hiz} KM/S | GECİKME: ${data.gecikme_ms}ms | SAAT: ${now}... \u00A0\u00A0\u00A0\u00A0 🟢;
                    }
                } catch (e) {
                    document.getElementById('led').innerText = "🔴 BAĞLANTI KOPTU VEYA SUNUCU UYKUDA...";
    ...
    """
    return HTMLResponse(content=html.replace("_OPTS_", opts))
...
async def mods(mod: str):
    ...
    <div style="color:white; font-weight:900;">YAPIM AŞAMASINDA</div></div>
